---
title: "Mjerenje uspješnosti investitora na financijskim tržištima"
output:
  html_notebook: default
  html_document:
    df_print: paged
  pdf_document: default
---

Učitavanje datasetova jednog po jednog te dodavanje godine kao stupac u svaki dataset. Spajanje svih datasetova u jedan veliki i micanje praznih redaka nastalih vjerovatno zbog moje pretvore u csv.
Nakon toga svi stupci koji u sebi imaju postotak su pretvoreni u double broj. Znak posto je maknut te broj koji se sada nalazi u tom stupcu označava postotak. Tipa ako piše 10 to onda znači 10 posto. Kolumna koja sadrži godine je potom pretovrena u factor, tj kategorijski podatak. Takav dataset je spremljen u file dionice.Rdata te ga se lako može učitati iz memorije kasnije.

```{r}
# dionice.2009 = read.csv("podaci/mojedionice2009.csv", header = T, sep = ",")
# dionice.2010 = read.csv("podaci/mojedionice2010.csv", header = T, sep = ",")
# dionice.2011 = read.csv("podaci/mojedionice2011.csv", header = T, sep = ",")
# dionice.2012 = read.csv("podaci/mojedionice2012.csv", header = T, sep = ",")
# dionice.2013 = read.csv("podaci/mojedionice2013.csv", header = T, sep = ",")
# dionice.2014 = read.csv("podaci/mojedionice2014.csv", header = T, sep = ",")
# dionice.2015 = read.csv("podaci/mojedionice2015.csv", header = T, sep = ",")
# dionice.2016 = read.csv("podaci/mojedionice2016.csv", header = T, sep = ",")
# dionice.2017 = read.csv("podaci/mojedionice2017.csv", header = T, sep = ",")
# 
# dionice.2009$Godina = 2009
# dionice.2010$Godina = 2010
# dionice.2011$Godina = 2011
# dionice.2012$Godina = 2012
# dionice.2013$Godina = 2013
# dionice.2014$Godina = 2014
# dionice.2015$Godina = 2015
# dionice.2016$Godina = 2016
# dionice.2017$Godina = 2017

# problem koji ovdje nastaje je taj da dionice iz 2009 imaju drugačiji naziv za otp indeks

# help("colnames")
# colnames(dionice.2010)[7]
# colnames(dionice.2009)[7] = colnames(dionice.2010)[7]
# dionice.2009$Prinos.iznad..OTP.indeksnog.fonda

# dionice = rbind(
#   dionice.2009,
#   dionice.2010,
#   dionice.2011,
#   dionice.2012,
#   dionice.2013,
#   dionice.2014,
#   dionice.2015,
#   dionice.2016,
#   dionice.2017
# )

# 
# dionice = na.omit(dionice)
# dionice
# 
# my_function("5.34%")
# my_function("5,34%")
# my_function = function(string) {
#   s = gsub("%", "", string)
#   s = gsub(",", ".", s)
#   return(as.numeric(s))
# }
# 
# dionice$Prinos.iznad..OTP.indeksnog.fonda = unlist(lapply(dionice$Prinos.iznad..OTP.indeksnog.fonda, my_function))
# dionice$Prinos = unlist(lapply(dionice$Prinos, my_function))
# dionice$Prinos.bez.div. = unlist(lapply(dionice$Prinos.bez.div., my_function))
# 
# save(dionice, file = "dionice.Rdata")
# load("dionice.Rdata")
# 
# head(dionice)
# 
# class(dionice$Prinos.iznad..OTP.indeksnog.fonda)
# class(dionice$Godina)
# 
# levels(dionice$Godina)
# head(dionice)
# dim(dionice)
# names(dionice)
# 
# dionice$Godina = as.factor(dionice$Godina)
# levels(dionice$Godina)

# save(dionice, file = "dionice.Rdata")
load("dionice.Rdata")

```

# Deskriptivna statistika. 
Proučavanje dobivenog dataseta.

Proučavanje po godinama.

Summary po svim godinama. Ne govori nam previše te je dosta teško usporedit podatke.
```{r}
tapply(dionice$Prinos.iznad..OTP.indeksnog.fonda, dionice$Godina, summary)
```

Graf srednje vrijednosti kroz godine. Većina srednjih vrijednosti se nalazi u blagome minusu uz izuzetak 2016 kada je srednja vrijednost na skoro 8 posto u plusu. 2010 godina je pak u minusu od 8%.

```{r}
mean.by.year = aggregate(dionice[, c(7)], list(dionice$Godina), mean)
# mean.by.year
plot(mean.by.year)
```


Graf mdeijana kroz godine. Većina medijana se isto nalazi u blagome minusu no zanimljivo je da najveći medijan se ovaj put nalazi u 2012 godini. Medijan za 2012 iznosi 5.63% dok srednja vrijednost za 2012 3.1%. 2010 godina je i dalje u velikom minusu i to skoro za 9% ovaj puta.

```{r}
median.by.year = aggregate(dionice[, c(7)], list(dionice$Godina), median)
# median.by.year
plot(median.by.year)
```


Boxplotovi za svaku godinu pokazani na istom grafu.
Vidimo da su svi negdje oko nule te počinjemo sumnjati u hipotezu da je tržište moguće pobijediti.
Primijećujemo stršeću pozitivnu vrijednost u 2013 godini. Neko je tad naime bio 131% u plusu.
Skrećemo pažnju na donji box u 2012 godini koji je osjetno duđi od gornjeg boxa te zato je takva negativna vrijednost te godine. Paralelno s tim možemo primijetit da je 2016 gornji box osjetno veći od donjeg boxa.

```{r}
require(ggplot2)
ggplot(dionice, aes(x=Godina, y=Prinos.iznad..OTP.indeksnog.fonda)) + geom_boxplot(aes(fill=Prinos.iznad..OTP.indeksnog.fonda))
```


Sve godine gledane zajedno.

Možemo pretpostaviti na temelju histograma da se stvarno radi o normalnoj distribuciji.

```{r}
hist(dionice$Prinos.iznad..OTP.indeksnog.fonda)
```

QQplot je dosta blizu pravca.

```{r}
qqnorm(dionice$Prinos.iznad..OTP.indeksnog.fonda)
```


Najlošiji postotak je onaj od -84% dok je najbolji od plus 131%.
Medijan i srednja vrijednost su jako blizu nule.

```{r}
summary(dionice$Prinos.iznad..OTP.indeksnog.fonda)
```

Interkvartilni rang iznosi 22 te vidimo gore da se nalazi između -10 i plus 10. Očekivano je da se 50 posto podataka nalazi oko nule uz ovakve medijane i srednje vrijednosti.
Varijanca je 403. Što vjerovatno znači da podaci dosta variraju.
```{r}
otp = dionice$Prinos.iznad..OTP.indeksnog.fonda
IQR(otp); var(otp)
```

Ako probamo izračunat mean ali maknemo 20 posto najmanjih i 20 posto najvećih srednja vrijednost je i dalje oko nule. Znači da stršeće vrijednosti definitivno ne utječu previše.

```{r}
mean(otp, trim=0.2)
```

Boxplot 

```{r}
boxplot(otp)
```

# Testiranje srednje vrijednosti otp-a

Testiramo null hipotezu da je srednja vrijednost jendaka nuli, s alternativom da nije jednaka nuli.
Alpha je 5% a p vrijednost je 0.58% što znači da možemo odbaciti null hipotezu.
Gledajući 95%-tni interval pouzdanosti koji ide od [-2.0, -0.33] možemo vidjet da nula definitvno nije unutra te da je srednja vrijednost otp-a populacije negativna. 
Možemo zaključiti kako pojedinci generalno gube a tržište pobjeđuje.

```{r}
t.test(otp, mu=0, alternative = "two.sided")
```

Testiranje srednje vrijednosti medijana svake godine zasebno.

```{r}
medians = tapply(dionice$Prinos.iznad..OTP.indeksnog.fonda, dionice$Godina, median)
medians
```

I ne možemo odbaciti nultu hipotezu u korist hipoteze da je medijan po godinama veći od nula. P vrijednost je 82%. Još jedan jak dokaz kako tržište pobjeđuje.

```{r}
t.test(medians, mu=0, alt="greater")
```



Ide li ljudima sve bolje i bolje.
Po ovom dolje izgleda da ide sve gore i gore jer mogu odbacit hipotezu da su iz 2009 i 2017 jednake srednje vrijednosti u korist hipoteze da im je zapravo bolje išlo 2009.

```{r}
otp_2017 = dionice[dionice$Godina == 2017, ]$Prinos.iznad..OTP.indeksnog.fonda
otp_2009 = dionice[dionice$Godina == 2009, ]$Prinos.iznad..OTP.indeksnog.fonda
t.test(otp_2009, otp_2017, alt="greater")
```


## FUN FACT: Broj prijavljenih ekipa raste tokom godina

```{r}
#pie chart
pie(table(dionice$Godina))
#barplot
barplot(table(dionice$Godina))
```


# Broj promjena portfelja

Parsiranje stupca s dionicama kako bi izvukli broj promjena tokom natjecanja. Taj stupac je potom dodan kao stupac Broj.promjena

```{r}
parser = function(string) {
  s = strsplit(as.character(string), "\\s+")[[1]][1]
  s = gsub("[(]", "", s)
  s = gsub("[)]", "", s)
  return(as.numeric(s))
}
dionice$Broj.promjena = dionice$Dionice
dionice$Broj.promjena = unlist(lapply(dionice$Broj.promjena, parser))

save(dionice, file = "dionice.Rdata")
```

Većina ljudi ne mijenja previše portfelj.

```{r}

hist(dionice$Broj.promjena)
```


Korelacija između stupca broj promjena i otp postotka je nažalost jako malena(pozitivna) te ne možemo tvrditi da broj promjena povećava šanse za uspjeh.

```{r}
cor(dionice$Prinos.iznad..OTP.indeksnog.fonda, dionice$Broj.promjena);
# cor(dionice$Prinos.iznad..OTP.indeksnog.fonda, dionice$Broj.promjena, method = "kendall")
# cor(dionice$Prinos.iznad..OTP.indeksnog.fonda, dionice$Broj.promjena, method = "spearman")
```

Većina ih je definitivno grupirana oko nule.

```{r}
plot(dionice$Broj.promjena, dionice$Prinos.iznad..OTP.indeksnog.fonda)
```

Ako pozovem pak summary nad njima onda vidim da broj promjena ipak utječe. Oni s 0, 1 ili 2 promjene su u većini no njihova srednja vrijednost je negativna do srednje vrijednosti svakog ostalog broja promjena >=3 su pozitivni.

```{r}
tapply(dionice$Prinos.iznad..OTP.indeksnog.fonda, dionice$Broj.promjena, summary)
```

Definitvno ide bolje onima s više promjena po ovome.

```{r}
plot(tapply(dionice$Prinos.iznad..OTP.indeksnog.fonda, dionice$Broj.promjena, mean), xlab = "Broj promjena", ylab = "Srednja vrijednost")
```


Ima ih nekoliko s pozitivnim medijanom također u većem brojem promjena.

```{r}
ggplot(dionice, aes(x=Broj.promjena, y=Prinos.iznad..OTP.indeksnog.fonda, group=Broj.promjena)) + geom_boxplot(aes(fill=Prinos.iznad..OTP.indeksnog.fonda))
```


Test usporedbe srednje vrijednosti onih s 0 promjena i onih s >=1 promjenom.
Alternativna hipoteza je ta da onima s više promjena ide bolje.
Ne možemo odbaciti nultu hipotezu jer je p vrijednost 99%, to znači da ovi koji su mijenjali dataset više puta nisu prošli ništa bolje od onih koji nisu mijenjali.

```{r}
nula_promjena = dionice[dionice$Broj.promjena > 0, ]$Prinos.iznad..OTP.indeksnog.fonda
više_od_nula_promjena = dionice[dionice$Broj.promjena == 0, ]$Prinos.iznad..OTP.indeksnog.fonda
length(nula_promjena) + length(više_od_nula_promjena) == length(otp)

t.test(nula_promjena, više_od_nula_promjena, alternative = "less")
```

# Ekipe koje se ponavljaju tokom godina

Nalazimo one čije se ime barem 2 puta ponovilo.
```{r}
name.occurence = data.frame(table(dionice$Naziv.portfelja))
more.than.2.times = name.occurence[name.occurence$Freq > 1, ]
```

U 2234 podataka ima samo 1347 jedinstevnih igrača te od tog ima 430 igrača koji su bili barem dva puta.
```{r}
length(name.occurence$Var1); length(more.than.2.times$Var1)
```

### Donosi li iskustvo prednost?

Za one koji imaju više sudjelovanja, sveli smo im otp na srednju vrijednost svih pokušaja.
Razdvojimo dataset na starosjedioce i guštere.
```{r}
spojeni_otp = aggregate(dionice$Prinos.iznad..OTP.indeksnog.fonda, by=list(dionice$Naziv.portfelja), FUN=mean)
colnames(spojeni_otp) = c("ID", "otp")

gusteri = spojeni_otp[!(spojeni_otp$ID %in% more.than.2.times$Var1), ]
starosjedioci = spojeni_otp[spojeni_otp$ID %in% more.than.2.times$Var1, ]
length(gusteri$ID); length(starosjedioci$ID)
```

Idemo malo proučit starosjedioce.
Izgleda da nisu ništa posebno. Srednja vrijednost i medijan blago negativni opet.

```{r}
boxplot(starosjedioci$otp)
```

Većina ljudi(ekipa) sudjeluje samo jednom.

```{r}
hist(dionice_broj_sudejlovanja$Broj.sudjelovanja)
```


Summary
```{r}
summary(starosjedioci)
```

Ide li starosjediocima bolje nego gušterima?

Ako napravimo test s alternativnom hipotezom da starosjediocima ide bolje dobijemo p vrijednost od 13.3% i ne možemo odbaciti nultu hipotezu.
95%-tni interval pouzdanosti je [-0.7461803  2.7013302] i sadrži nulu. Znači ima razlike među njima no ona nije dovoljno značajna da odbacimo hipotezu da su jednaki.

```{r}
t.test(starosjedioci$otp, gusteri$otp, alternative = "greater")
```

```{r}
t.test(starosjedioci$otp, gusteri$otp)
```

Možda je problem taj što smo starosjediocima uzimali srednju vrijednost njihovih pokušaja. Ajmo ić logikom da im ide sve bolje i bolje s iskustvom koje nakupljaju pa uze samo njihove zadnje pokušaje
(možemo i najbolje koje imaju to tad već varamo navijajući za njih).

```{r}
spojeni_otp_zadnja_vrijednost = aggregate(dionice$Prinos.iznad..OTP.indeksnog.fonda, by=list(dionice$Naziv.portfelja), FUN=function(x) tail(x, n=1)[1])
colnames(spojeni_otp_zadnja_vrijednost) = c("ID", "otp")

starosjedioci_zadnja_vrijednost = spojeni_otp_zadnja_vrijednost[spojeni_otp_zadnja_vrijednost$ID %in% more.than.2.times$Var1, ]
length(gusteri$ID); length(starosjedioci_zadnja_vrijednost$ID)
```

Nažalost ni tom im nije puno pomoglo. P vrijednost je 97% te alternativnu hipotezu da su starosjedioci bolji definitvno ne možemo shvatit ozbiljno.

```{r}
t.test(starosjedioci_zadnja_vrijednost$otp, gusteri$otp, alternative = "greater")
```


Ide li tim ljudima bolje kroz godine?
Nope.

```{r}
t.test(starosjedioci_zadnja_vrijednost$otp, starosjedioci$otp, alternative = "greater")
```


Postoji li korelacija između broja mijenjanja portfelja i frekvencije sudjelovanja na natjecanju? 
To jest mijenjaju li starosjedioci češće portfelj. 
Korelacija je 0.25. Nije jaka korelacija no starosjedioci ipak malo češće mijenjaju portfelj.

```{r}
colnames(name.occurence) = c("Naziv.portfelja", "Broj.sudjelovanja")
dionice_broj_sudejlovanja = merge(dionice, name.occurence, by="Naziv.portfelja")
dionice_broj_sudejlovanja = dionice_broj_sudejlovanja[with(dionice_broj_sudejlovanja, order(Godina, -Prinos)), ]

cor(dionice_broj_sudejlovanja$Broj.promjena, dionice_broj_sudejlovanja$Broj.sudjelovanja)
```

Šta je pak s korelacijom između broja sudjelovanja i prinosa.
Ona je 0.049. 
Možemo reć da ne postoji.

```{r}
cor(dionice_broj_sudejlovanja$Prinos.iznad..OTP.indeksnog.fonda, dionice_broj_sudejlovanja$Broj.sudjelovanja)
```


Najčešće odabrana dionica

```{r}
# Prvo moramo parsirati stupac Dionice i dobiti listu stringova(factora) koji predstavljaju dionice

parser = function(string) {
  s = strsplit(as.character(string), "\\s+")[[1]][2]
  return (unlist(strsplit(s, "[,]")))
}
parser(dionice$Dionice[1])
parser(factor(dionice$Dionice[1]))
dionice$Lista.Dionica = lapply(dionice$Dionice, parser)
dionice$Lista.Dionica[1]
head(dionice)
summary(dionice$Lista.Dionica)

install.packages("XML")
install.packages("RCurl")
install.packages("qdapTools")

library(qdapTools)
dionice[1]
binary_dionice = cbind(dionice$RB, dionice$Naziv.portfelja, mtabulate(dionice$Lista.Dionica))
binary_dionice

help("colSums")
# colSums(binary_dionice)
binary_dionice[, -c(1,2)] # omitting first and second column
binary_samo_dionice = binary_dionice[, -c(1, 2)]

colSums(binary_samo_dionice)
sort(colSums(binary_samo_dionice), decreasing = T)

```


parsiraj odabrane dionice pa koja je najbolja za imat
mjesec izrade računa ovisi li o nečemu 




Add a new chunk by clicking the *Insert Chunk* button on the toolbar or by pressing *Ctrl+Alt+I*.

When you save the notebook, an HTML file containing the code and output will be saved alongside it (click the *Preview* button or press *Ctrl+Shift+K* to preview the HTML file).

The preview shows you a rendered HTML copy of the contents of the editor. Consequently, unlike *Knit*, *Preview* does not run any R code chunks. Instead, the output of the chunk when it was last run in the editor is displayed.



